 ...
 *
 */
func insertNodeAtPosition(llist *SinglyLinkedListNode, d int32, position int32) *SinglyLinkedListNode {
	fmt.Println(d)
	// Create a new node with data
	newNode := &SinglyLinkedListNode{data: d, next: nil}

	fmt.Println("newNode Data: ", newNode.data)
	// Create a current node for data traversal
	currentNode := llist // set it to the head of the linkedlist
	// Create an index value initialised at 0
	var index = 0
	// while loop to get to one before insertion position
	for index < int(position)-1 {
		// Once at insertion point, set current node to the next node
		currentNode = currentNode.next
		// increment counter
		index++
	}
	// Point the new node to the current next value
